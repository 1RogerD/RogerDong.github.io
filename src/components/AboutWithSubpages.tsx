...
import { Anchor, Trophy, Code, Target, ArrowLeft } from 'lucide-react';
import SailingAchievementsPage from './SubPages/SailingAchievementsPage';
import RoboticsLeadershipPage from './SubPages/RoboticsLeadershipPage';
import SportsExcellencePage from './SubPages/SportsExcellencePage';
import AITechnologyPage from './SubPages/AITechnologyPage';

const AboutWithSubpages = () => {
  const [currentPage, setCurrentPage] = useState<string | null>(null);

  if (currentPage === 'sailing-achievements') {
    return <SailingAchievementsPage onBack={() => setCurrentPage(null)} />;
  }
  if (currentPage === 'robotics-leadership') {
    return <RoboticsLeadershipPage onBack={() => setCurrentPage(null)} />;
  }
  if (currentPage === 'sports-excellence') {
    return <SportsExcellencePage onBack={() => setCurrentPage(null)} />;
  }
  if (currentPage === 'ai-technology') {
    return <AITechnologyPage onBack={() => setCurrentPage(null)} />;
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid md:grid-cols-2 gap-12 items-center mb-16">
          ...
          <div className="grid grid-cols-2 gap-6">
            <div 
              className="text-center p-6 bg-red-900/80 rounded-lg border border-red-700 transition-all duration-300"
            >
...
export default AboutWithSubpages;